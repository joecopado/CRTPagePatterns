...
JS = r"""
;(function(){var U='http://127.0.0.1:%(port)d';window.__gzQ=Promise.resolve();window.__gzSeen={};window.__gzPending={};window.__gzPendingEl={};var HOLD=900;var SEQ=0;window.__gzAcked=[];
...
function gzHold(j,self,ev,xp){if(!j||!j.hold_ms)return;var id=j.hold_id;setTimeout(function(){enqueue(function(){return ask({gz_flush:true,hold_id:id}).then(function(k){gzOutcome(k);apply(k,self,ev,xp,null)})})},j.hold_ms)}
...
function apply(j,self,ev,xp,stock){((j&&j.replays)||[]).forEach(function(r){apply(r,self,ev,xp,null)});((j&&j.pre)||[]).forEach(function(pp){self.pushStep(pp,ev,xp)});var line=(j&&typeof j.line==='string')?j.line:stock;if(line!=null&&line!==''){self.pushStep(line,ev,xp)}((j&&j.backups)||[]).forEach(function(b){self.pushStep(b,ev,xp)});gzHold(j,self,ev,xp);gzAck(j)}
function recover(seq,self,ev,xp){return ask({gz_flush:true,lost:seq}).then(function(k){gzOutcome(k);var found=((k&&k.replays)||[]).some(function(r){return r.seq===seq});apply(k,self,ev,xp,null);return found})}
...
window.__gzCompose=function(self,r,event,ctx){window.__gzRec=self;var x=ctx?self.handler.getXPathForElement(ctx):undefined;
 ...
 enqueue(function(){var mine=++SEQ;return ask({seq:mine,rendered:r,xpath:x,alt_xpath:alt,alt:altKey,sentinel:sen,nonce:mark.nonce,descriptor:mark.descriptor}).then(function(j){gzOutcome(j);apply(j,self,event,x,r)}).catch(function(e){console.log('gz compose failed',e);return recover(mine,self,event,x).then(function(found){if(!found){self.pushStep(r,event,x)}}).catch(function(e2){console.log('gz recover failed',e2);self.pushStep(r,event,x)})})})};
function safetyNet(ev,kind){try{var tg=ev.composedPath?ev.composedPath()[0]:ev.target;if(!tg||tg.nodeType!==1)return;var el=tg;
 ...
 window.__gzPending[k]=setTimeout(function(){delete window.__gzPending[k];delete window.__gzPendingEl[k];if(window.__gzSeen[k]&&Date.now()-window.__gzSeen[k]<1500+HOLD)return;window.__gzSeen[k]=Date.now();
  enqueue(function(){var mine=++SEQ;return ask({seq:mine,rendered:'',xpath:k,synthetic:true,kind:kind,value:value,alt:altKey,sentinel:sen,nonce:mark.nonce,descriptor:mark.descriptor,tag:el.tagName.toLowerCase(),etype:(el.type||''),text:(el.textContent||'').trim().slice(0,80)}).then(function(j){gzOutcome(j);apply(j,self,ev,k,null)}).catch(function(e){console.log('gz net failed',e);return recover(mine,self,ev,k).catch(function(e2){console.log('gz recover failed',e2)})})})},HOLD)}catch(e){}}
...
"""
STATE = {"version": "2026-09-20n11 faker", "dormant_form": "keyword", "form": "keyword", "org": None, "patched": None,
...
def _pick_dormant_open(cl, label, line, indent, family):
    ...
    if fn is not None:
        try:
            ...
            pass
    return "%s#   backup: %s   (opened, no pick recorded -- not a step)" % (indent, (line or "").strip())
...
_REPLIES = {}          # seq -> {pre, line, backups, hold_ms, hold_id}, until the page acks it
REPLIES_CAP = 16
...
def _remember(seq, reply):
    if seq is None:
        return
    _REPLIES[seq] = {k: reply.get(k) for k in ("pre", "line", "backups", "hold_ms", "hold_id")}
    while len(_REPLIES) > REPLIES_CAP:
        _REPLIES.pop(min(_REPLIES))


def _replays(seq):
    """Every stored reply older than this ask that the page never acknowledged, in order."""
    if seq is None:
        return []
    out = [dict(_REPLIES[s], seq=s) for s in sorted(_REPLIES) if s < seq]
    if out:
...
class _H(BaseHTTPRequestHandler):
    ...
    def _compose(self, req):
        ...
        _ack(req)
        seq = req.get("seq")
        replays = _replays(seq)     # every older reply the page never acknowledged (F191)
        STATE["served"] += 1
        rendered = req.get("rendered") or ""
        xp = req.get("xpath")
        desc = req.get("descriptor") or {}
        ...
        _remember(seq, reply)   # BEFORE the send: a reply written to a dead socket is still kept
        self._send(dict(reply, outcome=_outcome_payload(), outcome_id=GZ_OUTCOME_ID,
                        outcome_attr=GZ_OUTCOME_ATTR))
