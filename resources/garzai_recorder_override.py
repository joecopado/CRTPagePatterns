...
function gzHold(j,self,ev,xp){if(!j||!j.hold_ms)return;var id=j.hold_id;setTimeout(function(){enqueue(function(){return ask({gz_flush:true,hold_id:id}).then(function(k){gzOutcome(k);((k&&k.pre)||[]).forEach(function(pp){self.pushStep(pp,ev,xp)})})})},j.hold_ms)}
...
window.__gzCompose=function(self,r,event,ctx){window.__gzRec=self;var x=ctx?self.handler.getXPathForElement(ctx):undefined;
 ...
 enqueue(function(){return ask({rendered:r,xpath:x,alt_xpath:alt,alt:altKey,nonce:mark.nonce,descriptor:mark.descriptor}).then(function(j){gzOutcome(j);var line=(j&&typeof j.line==='string')?j.line:r;((j&&j.pre)||[]).forEach(function(pp){self.pushStep(pp,event,x)});if(line!==''){self.pushStep(line,event,x)}((j&&j.backups)||[]).forEach(function(b){self.pushStep(b,event,x)});gzHold(j,self,event,x)}).catch(function(e){console.log('gz compose failed',e);self.pushStep(r,event,x)})})};
...
"""
STATE = {"version": "2026-09-20n9 engaged", "dormant_form": "keyword", "form": "keyword", "org": None, "patched": None,
...
class _H(BaseHTTPRequestHandler):
    ...
    def _flush(self, req):
        ...
        want = req.get("hold_id")
        pre = []
        if _SUP_HOLD["armed"] and (want is None or want == _SUP_HOLD["id"]):
            pre = [_dormant_form(x) for x in _sup_release()]
        self._send({"line": "", "backups": [], "pre": pre, "flushed": len(pre),
                    "outcome": _outcome_payload(), "outcome_id": GZ_OUTCOME_ID,
                    "outcome_attr": GZ_OUTCOME_ATTR})

    def _compose(self, req):
        if req.get("gz_flush"):
            return self._flush(req)
        STATE["served"] += 1
        rendered = req.get("rendered") or ""
        xp = req.get("xpath")
        desc = req.get("descriptor") or {}
