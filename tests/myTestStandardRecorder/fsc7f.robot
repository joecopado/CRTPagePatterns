...
Status Before Recording
    ...
    ClickText    Apps    partial_match=False
    ClickCheckbox    Actionable Segmentation    on    partial_match=False
    ClickCheckbox    Shared App    on    partial_match=False
    

    ${lead_first_name}=    First Name
    Omni Type    FSC_DL_v1_First_Name    ${lead_first_name}    omni-text    # unverified: parser proposal
    Comment    backup: TypeText    First Name    asdf    # unverified: parser proposal; stock recorder line, unverified
    Comment    backup: TypeText    xpath\=//input[@placeholder\="Enter your first name"]    asdf    # xpath form, unverified: parser proposal
    Comment    verify: Verify Input Value    First Name    ${lead_first_name}
    
    ClickText    Open actions menu for the Actionable Segmentation App
    ClickText    Details
    ClickText    Close asset details panel
    ClickText    Clear
    ClickText    Add Collection
    ${lead_name}=    Name
    TypeText    *Name    ${lead_name}
    Comment    verify: Verify Input Value    *Name    ${lead_name}
    ClickText    \#1b96ff
    ClickText    \#45c65a    anchor=\#01c3b3
    ClickText    Done
