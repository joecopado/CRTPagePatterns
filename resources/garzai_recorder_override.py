...
"""
STATE = {"version": "2026-09-19n8 routed", "dormant_form": "keyword", "form": "keyword", "org": None, "patched": None,
...
def _generated_data(req, decision, line, backups):
    ...
    taken = _taken()
    pre = []
    # F143-8 (F146-8): `decision["supersedes"]` was a DIAGNOSTIC key only -- the page's own
    # `pushStep` only ever APPENDS, so a typed sentinel corrected by a literal (or the reverse) on
    # the SAME control left BOTH lines in the pane with nothing saying either was overwritten, and
    # a teardown keyed on the generated run stamp then matches no record. REMOVING an already-pushed
    # step needs `pushStep` itself to support it, which is unproven without a live CRT Live Testing
    # container (COULD-NOT-CHECK without one -- BACKLOG); what this build CAN prove offline is
    # turning the SILENT stranding into a DISCLOSED one: a dormant line, ABOVE this event's own
    # line, naming both values and which one is the later, committed evidence.
    sup = decision.get("supersedes")
    if sup:
        indent = (line[: len(line) - len(line.lstrip())] or "    ")
        pre.append("%s#   supersedes: %s" % (indent, sup))
    try:
    ...
    if not var_line:
        ...
        # F143-5 (F146-5): `new_line` carries a COULD-NOT-CHECK mark ON THE LINE ITSELF when the
...
def _omni_pair(req, decision, line, backups):
    ...
    if _OMNI_HOLD["held"] and cl.omni_combobox_option(rendered, *paths):
        ...
        # combobox's own filter box (build n8, F143-3/F143-4): a click-recorded control cannot
    ...
    if _OMNI_HOLD["held"]:
        ...
        # keystroke report) -- build n8, F143-3/F143-4. The filter box is not where a generated
        ...
        if is_sentinel and same_host is not False:
            ...
            decision["omni"] = ("a typed sentinel %r inside the held combobox's own input (%s "
                                ...
                                "composes a Gz Pick pair instead of a hard-coded literal (F143-3/"
                                "F143-4) -- nothing is typed into the filter box itself"
